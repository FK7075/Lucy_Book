...
public class AdminAction extends ActionSupport {
	...
	public String bookUpload() {
		...
		try {
			...
			FileUtils.copyFile(bookUp, new File(file, fileName));//把流写到文件夹里     图片从本机上传到服务器
	...
	public String showStort() {
		int page=getPage("pages");
		PageInfo pi = new PageInfo(Stort.class,new Stort(),page,sessionAdmin.getPagesize());//分页所需要的信息
		if (page == pi.getPagenum())
			request.setAttribute("fk", 1);
		request.setAttribute("page", page);
	...
	public String showAuthorInfo() {
		int id=0; // 
		if(request.getParameter("id")!=null) //拿到string类型的id
			id=Integer.parseInt(request.getParameter("id"));
		if(request.getAttribute("id")!=null)
			id=(int) request.getAttribute("id"); //拿到object类型id
	...
	public String noPayment() {
		...
		PageInfo pi = new PageInfo(Orders.class,page,sessionAdmin.getPagesize(),sql1,"未付款");
		if (page == pi.getPagenum())
			request.setAttribute("fk", 1);
		request.setAttribute("page", page);
	...
	public String noDelivery() {
		...
		PageInfo pi = new PageInfo(Orders.class,page,sessionAdmin.getPagesize(),sql1,"已付款","未发货");
		if (page == pi.getPagenum())
			request.setAttribute("fk", 1);
		request.setAttribute("page", page);
